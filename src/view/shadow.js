    ...
    if (mode == MODE.LINE) {
      ...
      if (buf.length == 4) {
        this.observer.putDrawing(
          [buf[0], buf[1], buf[0], buf[3], buf[2], buf[1], buf[2], buf[3]],
          gl.TRIANGLE_STRIP,
          color,
        )
        this.buf = []
      ...
      if (this.buf.length > 4) {
        ...
        if (
          isSamePointWithTolerance(
            this.buf[0],
            this.buf[length - 2],
            tolerance,
          ) &&
          isSamePointWithTolerance(this.buf[1], this.buf[length - 1], tolerance)
        ) {
          //  Optional: End and start on the same point,remove duplicate
          this.buf = this.buf.slice(0, length - 2)

          this.observer.putDrawing(this.buf, gl.TRIANGLE_FAN, color)
          this.buf = []
        }
      }
    }
